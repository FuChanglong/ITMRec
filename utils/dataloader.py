...
class TrainDataLoader(AbstractDataLoader):
    ...
    def computer_neighbour_item(self):

        num_users = self.A.shape[0]
        interaction_items = [[] for _ in range(num_users)]

        for user in range(num_users):
            interacting_users = self.A.row[self.A.col == user]
            for interacting_user in interacting_users:
                interaction_items[user].extend(self.history_items_per_u[interacting_user])

        self.hidden_item_per_user=interaction_items
